...
<p align="center">
    <img src="./img/ind_part_bag.png" alt="indexes" />
</p>
...
Или можно представить, что у кубиков еще есть цвета:

<p align="center">
    <img src="./img/ind_part_three_bags.png" alt="indexes" />
</p>
...
<p align="center">
    <img src="./img/ind_clust_vs_non_clust.png" alt="indexes" />
</p>
...
### Включаемые поля (INCLUDE) 
PostgreSQL поддерживает расширение B‑tree индексов за счёт включаемых полей. Такие поля не участвуют в сортировке и поиске, но сохраняются в листьях индекса. Это позволяет выполнять index‑only scan и покрывать запросы, не обращаясь к таблице.

```
CREATE INDEX idx_orders_order_date_inc
...
Индекс может строиться не по колонке напрямую, а по выражению или функции. Это обеспечивает ускорение запросов, где используется одинаковое выражение.
...
Индекс может охватывать только часть строк таблицы, если задано условие WHERE. Это уменьшает размер индекса и снижает нагрузку на операции записи.
...
Возможна комбинация функционального выражения, условия и включаемых полей в одном индексе.

```
CREATE INDEX idx_orders_completed_month_inc
...
1. Включаемые поля применяются для покрытия запросов, но не влияют на уникальность индекса.
2. Функциональные индексы работают только при точном совпадении выражения в запросе и индексе.
3. Частичные индексы эффективны при высокой селективности условия.
4. Каждый индекс увеличивает стоимость операций INSERT/UPDATE/DELETE, поэтому их следует создавать исходя из реальных сценариев запросов.
...
<p align="center">
    <img src="./img/ind_btree.png" alt="indexes" />
</p>
...
<p align="left">
    <img src="./img/ind_btree_revers1.png" alt="indexes" />
</p>
...
<p align="center">
    <img src="./img/ind_btree_revers2.png" alt="indexes" />
</p>
...
<p align="center">
    <img src="./img/ind_hash.png" alt="indexes" />
</p>
...
<p align="center">
    <img src="./img/ind_bitmap1.png" alt="indexes" />
     <img src="./img/ind_bitmap2.png" alt="indexes" />
</p>
...
<p align="center">
    <img src="./img/ind_bitmap3.png" alt="indexes" />
</p>
...
<p align="center">
    <img src="./img/partitions.png" alt="indexes" />
</p>
